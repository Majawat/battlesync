...
- [x] **FIXED: Upgrade chain processing** - ~~Sgt. Pistol → Drum Pistol and Sgt. Hand Weapon → Energy Axe chains not working~~ - **RESOLVED in v2.19.1**: Fixed critical weapon ID matching issue in upgrade chains. Enhanced dependency system to handle both `id` and `weaponId` properties from upgrade gains. Sergeant weapon chains now work correctly: Rifle+CCW → Sgt. Pistol+Sgt. Hand Weapon → Drum Pistol/Plasma Pistol + Energy Axe/Energy Sword.
...
- [x] **FIXED: Campaign upgrade duplication** - ~~Campaign upgrades being doubled (Company Standard, Field Radio, Medical Training)~~ - **RESOLVED in v2.18.0**: Implemented grouped upgrade processing to handle multi-option upgrade sections correctly. Upgrades that affect multiple models now distribute different options across models instead of duplicating the same upgrade on each affected model.
...
- `GET /api/battleaura/firmware/download/:filename` - Download firmware binary files
